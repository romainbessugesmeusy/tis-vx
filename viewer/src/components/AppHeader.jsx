...
function AppHeader({
  ...
  // Page-level bookmark: determine if current route is bookmarkable and build prefill data
  const pageBookmarkPrefill = useMemo(() => {
    const path = location.pathname

    if (path.startsWith('/doc/')) {
      const slug = path.slice(5)
      const section = manifest?.sections?.find(
        s => s.id === slug || Object.values(s.variants || {}).some(v => v?.slug === slug)
      )
      return {
        type: 'manual',
        title: section?.title || breadcrumb[breadcrumb.length - 1]?.label || slug,
        route: path,
        context: { slug },
      }
    }

    if (path.match(/^\/epc\/[^/]+\/diagram\/[^/]+$/)) {
      const parts = path.split('/')
      const groupId = parts[2]
      const diagramId = parts[4]
      return {
        type: 'epc-diagram',
        title: breadcrumb[breadcrumb.length - 1]?.label || `Diagram ${diagramId}`,
        route: path,
        context: { groupId, diagramId },
      }
    }

    return null
  }, [location.pathname, manifest, breadcrumb])

  const isCurrentPageBookmarked = pageBookmarkPrefill
    ? isBookmarked(pageBookmarkPrefill.type, pageBookmarkPrefill.route, pageBookmarkPrefill.context)
    : false

  const handleBookmarkPage = useCallback(() => {
    if (pageBookmarkPrefill) {
      openBookmarkDialog(pageBookmarkPrefill)
    }
  }, [pageBookmarkPrefill, openBookmarkDialog])
  ...
  return (
    <header className="header" ref={headerRef}>
            ...
            {activeDropdown === 'nav' && (
              <div className="header-popover nav-popover">
                <div className="popover-section popover-nav">
                  ...
                  <button
                    className={`popover-nav-item ${location.pathname === '/bookmarks' ? 'active' : ''}`}
      ...
      <div className="header-right">
        {/* Page-level bookmark button */}
        {pageBookmarkPrefill && (
          <button
            className={`header-nav-btn header-bookmark-btn ${isCurrentPageBookmarked ? 'bookmarked' : ''}`}
        ...
        {!showMobileMenu && (
          <button
            className={`header-nav-btn ${location.pathname === '/bookmarks' ? 'active' : ''}`}
            onClick={() => handleNav('/bookmarks')}
            title="Bookmarks"
          >
            <IconBookmark filled={false} />
